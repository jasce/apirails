class AddConfirmableToStore < ActiveRecord::Migration
  def up
    ...
    # store.reset_column_information # Need for some types of updates, but not for update_all.
    # To avoid a short time window between running the migration and updating all existing
    # stores as confirmed, do the following
   # execute("UPDATE stores SET confirmed_at = NOW()")
    # All existing store accounts should be able to log in after this.
    # Remind: Rails using SQLite as default. And SQLite has no such function :NOW.
    # Use :date('now') instead of :NOW when using SQLite.
     execute("UPDATE stores SET confirmed_at = date('now')")
    # Or => store.all.update_all confirmed_at: Time.now
  end
